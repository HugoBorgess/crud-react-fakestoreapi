  ...
  return (
    <section>
      <div className={stylesTema.titulo}>
        <h2 className={stylesTema.titulo__texto}>Usuários - FAKESTORE API</h2>
        <button
          type='button'
          id='cadastrar'
          name='cadastrar'
        ...
        {usuarios.map(usuario => (
          ...
          >
            <div className={styles.usuario__name}>
              {`${usuario.name.firstname.toUpperCase()} ${usuario.name.lastname.toUpperCase()} - ${usuario.email.toUpperCase()}`}
            </div>  
          </div>
        ))}
      </div>
